...
lrGeneratorMixin.generateModule = function generateModule (opt) {
    opt = typal.mix.call({}, this.options, opt);
    var moduleName = opt.moduleName || "parser";
    var exportAsConstructor = opt.exportAsConstructor || false;
    ...
    out += (moduleName.match(/\./) ? moduleName : "var "+ moduleName) +
            " = "  (exportAsConstructor ? "" : "new ") + this.generateModuleExpr();
...
lrGeneratorMixin.generateModuleExpr = function generateModuleExpr () {
    ...
    out += module.sharedCode;
    out += "\nvar proto = "+module.moduleCode;
    out += "\n"+this.moduleInclude;
    if (this.lexer && this.lexer.generateModule) {
        out += this.lexer.generateModule();
    }
    out += "\nfunction Parser (options) {\n  var yy = {};\n  var parseError = " + module.parseError + ";\n"
    	//+ "  //var lexer = new Lexer();\n"
    	+ "  if (this instanceof Parser) {\n"
    	+ "    if (options && (Object.prototype.toString.call( options ) === \"[object Object]\")) {\n"
    	+ "      if (options['yy']) {\n"
    	+ "        yy = options['yy'];\n" // optionally copy in options['yy'], so that the sharedState var is no longer needed
    	+ "      }\n"
    	+ "      if (options['lexer']) {\n" // && (typeof options['lexer'] === '[object Lexer]')
    	+ "        lexer = options['lexer'];\n"
    	+ "      }\n"
    	...
    	+ "  }\n"
    	+ "  lexer = Object.create(lexer);\n" // Moved this line from parse
    	+ "this.__proto__['parse'] = " + module.parseCode + "\n"
    	+ "}\n"
...
function removeTokenStack (fn) {
	var parseFn = fn;
	return parseFn.replace(/_token_stack:\s?/,"");
}
...
lrGeneratorMixin.generateModule_ = function generateModule_ () {
    ...
    if (this.options['token-stack']) {
      parseFn = addTokenStack(parseFn);
    	...
    	parseFn = removeTokenStack(parseFn);
    }
    ...
    // While on the surface it looks "neat', there appears to be a performance hit for large grammars
    // by not including this in the table_ variable
    var commonCode = tableCode.commonCode;

	var sharedCode = ["var symbols_ = " + JSON.stringify(this.symbols_),
		"terminals_ = " + JSON.stringify(this.terminals_).replace(/"([0-9]+)":/g,"$1:"),
		"productions_ = " + JSON.stringify(this.productions_),
		"table_ = " + tableCode.moduleCode,
    ...
    moduleCode += "};";
    var parseCode = parseFn;
    // FIXME: the parseError function needs to be nameless due to IE bug
    var parseErrorCode = String(this.parseError || (this.hasErrorRecovery ? traceParseError : parser.parseError));

    return { commonCode: commonCode, moduleCode: moduleCode, parseCode: parseCode, parseError: parseErrorCode, sharedCode: sharedCode }
};

// Generate code that represents the specified parser table
lrGeneratorMixin.generateTableCode = function (table) {
	var opt = typal.mix.call({}, this.options, opt);
	var commonTableCode = (opt.commonTableCode === false ? opt.commonTableCode : opt.commonTableCode || true);
	...
	if (!commonTableCode) {
		return {commonCode: "", moduleCode: moduleCode};
	}
...
parser.parse = function parse (input, data) {
    var self = this,
        stack = [0],
        tstack = [], // token stack
...
_handle_error:
        ...
        switch (action[0]) {
            ...
            case 2:
                ...
                r = performAction.apply(yyval, [yytext, yyleng, yylineno, sharedState.yy, action[1], vstack, lstack, parseError].concat(args));
                ...
                vstack.push(yyval.$);
                lstack.push(yyval._$);
                // goto new state = table_[STATE][NONTERMINAL]
